...
<script type="text/javascript" src="js/image_preview.js"></script>
     ...
     /* function app_info()
      {
      var s1=document.forms['appinfoform']['apppass'].value
	  var s2=document.forms['appinfoform']['reapppass'].value
      if (s2!='' && s1!=s2)
      {
       alert('Sorry! Password and Retype Password do not match');
   
       document.forms['appinfoform']['reapppass'].style.background = 'Yellow';
  ...
  <SCRIPT LANGUAGE="JavaScript">
           function DisBox() 
               {
                if(document.appinfoform.disb.checked)
                  {
                   document.appinfoform.ETO_D1.blur()     
                   document.appinfoform.ETO_D1.disabled=true                  
                  }
               else
                  {
                   document.appinfoform.ETO_D1.disabled=false
                  }
               }   
  </SCRIPT>
...
<?php
	...
	$month_array=array("01"=>"January","02"=>"February","03"=>"March","04"=>"April","05"=>"May","06"=>"June","07"=>"July","08"=>"August","09"=>"September","10"=>"October","11"=>"November","12"=>"December");	
     list($app_y,$app_m,$app_d)=explode("-",$rows01[9]);
	 $app_month=$month_array[$app_m];
	 $app_birth_day=$app_d." ".$app_month." ".$app_y;
	...
	$result0301=mysql_query("SELECT * FROM app_scholastic_status WHERE app_id='$app_id' and app_position=1");
	...
	$result0302=mysql_query("SELECT * FROM app_scholastic_status WHERE app_id='$app_id' and app_position=2");
	...
	$result0303=mysql_query("SELECT * FROM app_scholastic_status WHERE app_id='$app_id' and app_position=3");
	...
	$result0304=mysql_query("SELECT * FROM app_scholastic_status WHERE app_id='$app_id' and app_position=4");
    $rows0304=mysql_fetch_array($result0304);
    $noe_msc_array=array("","MBA","MBM","MBS","MCom","M.Pharm","MSc","MSc.Ag","MSS","MA","MS","LLM","Others");
	
	
	$result04=mysql_query("SELECT * FROM app_extra_status WHERE app_id='$app_id'");
	...
	$result05=mysql_query("SELECT * FROM app_experience WHERE app_id='$app_id'");
	...
	$app_type_array=array("","Part Time","Permanent","Contractual");
	...
	$result0601=mysql_query("SELECT * FROM app_references WHERE app_id='$app_id' and position=1");
    $rows0601=mysql_fetch_array($result0601);
	$result0602=mysql_query("SELECT * FROM app_references WHERE app_id='$app_id' and position=2");
    $rows0602=mysql_fetch_array($result0602);
	
	
	
?>
<div style="width:980px; min-height:700px; margin:-15px 10px 10px 10px;"><!--Page Area -->
 <div>
<div  style=" width:980px; min-height:700px; text-align:left; line-height:25px; background:#CEE7FF;">
<div align="center" style="color: #F00;  font-family:Tahoma, Geneva, sans-serif; font-size:12px;"><?php echo @$_GET['message'];?></div>

        <div>
        	<div style="background:#005288; height:32px;"><h2 style="color:#FC0; margin:0 0 0 20px; text-align:center;">Employee Resume Edit</h2></div>
    ...
    <form  name="appinfoform" id='contactus' action="insert_app_resume_edit.php" method="post" enctype="multipart/form-data" onsubmit="return app_info()">
        ...
        <ul class="form-list" style="list-style:none;">
                    <li>
                        <div>
                            ...
                            <div id="second-side-style01">
                                <input name="first_name" id="first_name" value="<?php echo $rows01[2];?>" type="text">
                            </div>
					...
					<li>
                         	<div>
                                ...
                                <div id="second-side-style01">
                              		<input name="email" id="email" value="<?php echo $rows01[8];?>" class="input-text email required valid" type="text">
                                </div>
                                <div style="clear:both;"></div>
                            </div>
					</li>
      ...
      <td colspan="4" class="rightmenu" align="center"><table style="border:0px;" border="0" width="100%">
        ...
        <tr>
          ...
          <td align="left"><select class="text" name="EXP_TYPE1" id="EXP_TYPE1">
            <option value="<?php echo $rows05[3];?>" selected="selected"><?php echo $app_type_array[$rows05[3]];?></option>
        ...
        <tr>
		  ...
		  {
		  ?>
            <input name="ETO_D1" value="" class="text" id="ETO_D1" placeholder="yyyy-mm-dd"  type="date" onFocus="DisBox()">
            or Till now:<INPUT TYPE="checkbox" NAME="disb" value="1" checked="checked" onClick="DisBox()"> 
          <?php
		  }
		  else
		  {
		  ?>
             <input name="ETO_D1" value="<?php echo $rows05[14];?>" class="text" id="ETO_D1" placeholder="yyyy-mm-dd"  type="date" onFocus="DisBox()">
        ...
        <div style="margin:10px 0 10px 10px; width:960px; min-height:100px;">
        <div>
       ...
       <div><input type="file" name="image_file" id="image_file" onchange="fileSelected();" /></div>
        ...
        <img src="../../appimage/<?php echo $rows02[2];?>" height="150" width="150"  border="0"/></div>
        <img id="preview" />
        <div id="fileinfo">
            <div id="filename"></div>
            <div id="filesize"></div>
       ...
       <!--Code for image preview End-->
        ...
        <!--
          <table width="100%"><tbody>
            <tr>
               <td colspan="2"><span style="color:#FF4A4A; font-size:13px; line-height:15px; font-style:italic;">Please, provide a password. Password should be at least 6 character. Max 12 character. No space allowed.</span></td>
            </tr>
            <tr>
               <td width="28%">Password<span class="errorind">*</span></td>
               <td><input type="password" class="apppass" id="apppass" name="apppass" maxlength="12"/></td>
            </tr>
            <tr>
               <td>Retype Password<span class="errorind">*</span></td>
               <td><input type="password" class="reapppass" id="reapppass" name="reapppass" maxlength="12"/></td>
            </tr>
          </tbody></table>
          -->
        </div>
