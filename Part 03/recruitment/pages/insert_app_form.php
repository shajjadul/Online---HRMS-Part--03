...
//########### PASSWORD ###############
	...
	$conn->connection();
	 ...
	 $result = mysql_query("SELECT * FROM app_personal_info WHERE app_email='$email'");
	 ...
	 if(!$gg)
	 {
			...
			//########### PERSONAL INFORMATION ###############
			$table01="app_personal_info";
			
			$array01=array("$table_id","$app_id","$name","$father_name","$mother_name","$present_address","$permanent_address","$home_phone","$email","$birth_date","$nationality","$national_id_no","$religion","$marital_status","$gender","$blood_group","$submit_moment");
			
			$query->normal_insert($table01,$array01);
			
				
			//########### SCHOLASTIC STATUS ###############
			$table02="app_scholastic_status";
			
			$array0201=array("$table_id01","$app_id","$noe_ssc","$gs_ssc","$res_ssc","$gpa_ssc","$gpa_outof_ssc","$bu_ssc","$pass_ssc","1");
			$query->normal_insert($table02,$array0201);
			
			$array0202=array("$table_id02","$app_id","$noe_hsc","$gs_hsc","$res_hsc","$gpa_hsc","$gpa_outof_hsc","$bu_hsc","$pass_hsc","2");
			$query->normal_insert($table02,$array0202);
			
			$array0203=array("$table_id03","$app_id","$noe_bsc","$gs_bsc","$res_bsc","$gpa_bsc","$gpa_outof_bsc","$bu_bsc","$pass_bsc","3");
			$query->normal_insert($table02,$array0203);
			
			$array0204=array("$table_id04","$app_id","$noe_msc","$gs_msc","$res_msc","$gpa_msc","$gpa_outof_msc","$bu_msc","$pass_msc","4");
			$query->normal_insert($table02,$array0204);
			
			
			
			$table03="app_extra_status";
			
			$array03=array("$table_id","$app_id","$extracurriculamactivities","$otherskills");
			
			$query->normal_insert($table03,$array03);
			
			
			//########### EXPERIENCE ###############
			$table04="app_experience";
			
			$array04=array("$table_id","$app_id","$COMPANY1","$EXP_TYPE1","$EXP_AREA1","$EXP_POST1","$DEPT1","$UNIT1","$JOB_LOC1","$SUPER_NAME1","$SUPER_PHONE1","$SUPER_EMAIL1","$RESERVATION1","$EFROM_D1","$ETO_D1","$DUTY1","$ACHIEVE1","$CLOCATION1");
			
			$query->normal_insert($table04,$array04);
			
			
			//########### REFERENCES ###############
			$table05="app_references";
			
			$array0501=array("$table_id01","$app_id","$R_NAME1","$R_POS1","$R_COMP1","$R_ADD1","$R_PHONE1","$R_CELL1","$R_EMAIL1","$R_KNOW1","1");
			$query->normal_insert($table05,$array0501);
			$array0502=array("$table_id02","$app_id","$R_NAME2","$R_POS2","$R_COMP2","$R_ADD2","$R_PHONE2","$R_CELL2","$R_EMAIL2","$R_KNOW2","2");
			...
			$array06=array("$table_id","$app_id","$picture_name");
			...
			header("Location:app_resume_show.php?$mess");*/
			exit(header("Location:app_resume_show.php?app=$app_id"));
	
	 }
	 else
	 {
			$mess="message=Sorry! Your email ID already exists in database. Try another";
			exit(header("Location:app_info_form.php?$mess"));
	 }
